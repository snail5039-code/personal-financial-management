...
| **메일 · 일정** | "오늘 중요한 메일하고 일정 정리해줘" (Gmail·구글 캘린더, 둘 다 읽기 전용) |
...
| **되돌리기** | "방금 그거 취소해줘" (가계부·할일·메모·파일 이동/이름변경 전부) |
...
앱을 강제로 끄거나, 파일을 옮기고, 이름을 바꾸고, 지우는 것처럼 되돌리기 어려운 동작은
실행 전에 터미널에서 `진행할까요? (y/N)`로 먼저 확인받습니다. 그냥 앱을 켜거나 파일을
검색·열기·복사하는 것처럼 안전한 동작은 바로 실행됩니다. 파일 삭제는 완전히 지우지 않고
휴지통으로 보내서, 확인을 거쳤더라도 한 번 더 복구할 여지를 남깁니다.
...
- **다운로드 완료 알림**: 다운로드 폴더에 새 파일이 생기면(다운로드 도중 임시 파일은 무시하고
  완료된 것만) 알림을 보냅니다.
- **음성 웨이크워드**: "커리마"라고 부르면 삐 소리가 나고 이어서 하는 말을 듣습니다. 대화형
  모드처럼 도구를 호출해 답하고, 답은 알림 + 음성(한국어 TTS)으로 옵니다. 앱을 강제로 끄거나
  파일을 지우는 것처럼 확인이 필요한 동작은 음성에서는 실행하지 않고 "커리마를 열어서 직접
  말씀해주세요"라고 안내합니다. 트레이 아이콘을 우클릭하면 켜고 끌 수 있습니다.
...
트레이 아이콘을 우클릭하면 **"커리마 열기"**(새 창에서 `app.py` 실행)와 **"종료"**가 있습니다.
컴퓨터를 켤 때마다 자동으로 뜨게 하고 싶다면, `tray_app.py`의 바로가기를 `shell:startup` 폴더에
넣으면 됩니다 (윈도우 시작 프로그램 등록은 사용자가 직접 하는 설정이라 스크립트가 알아서 하지 않습니다).
...
둘 다 읽기 전용입니다 (메일 발송/삭제, 일정 등록/수정은 지원하지 않음). 위에서 만든 것과
**같은 Google Cloud 프로젝트, 같은 `credentials.json`을 그대로 재사용**하고, 필요한 API만
추가로 켜면 됩니다.
...
│  ├─ tools_calendar.py 구글 캘린더 (읽기 전용)
...
- **Gmail·구글 캘린더는 읽기 전용**입니다. 메일 발송/삭제, 일정 등록/수정은 지원하지 않고, 권한 토큰도
  할일과 따로 저장돼서(`data/google_gmail_token.json`, `data/google_calendar_token.json`) 서로 영향을 안 줍니다.
- **되돌리기는 한 단계**만 지원합니다.
