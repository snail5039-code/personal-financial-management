...
- **대화 기억**: `client.interactions.create(..., store=True)`로 매 턴을 Gemini 서버에 저장하고, 응답으로 받은 `interaction.id`를 다음 호출의 `previous_interaction_id`로 넘겨서 이어감 → 대화 내용 자체는 서버가 들고 있고, 우리는 id 문자열 하나만 들고 있으면 됨 (단, 이 id는 프로그램을 껐다 켜면 사라지므로 "한 번 실행하는 동안만 기억"하는 건 이전 방식과 동일)
...
원본 기획은 7개였으나, 대화 중 "예산만 등록", "새 카테고리 허용" 요구가 나오면서 8~10번이 추가됨.
...
- 결과 파일(JSON 내보내기, 월별 보고서)은 항상 프로젝트 루트의 `data/` 폴더에 저장된다.
